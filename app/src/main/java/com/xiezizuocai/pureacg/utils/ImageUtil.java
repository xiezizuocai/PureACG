...
public class ImageUtil {
    public static Uri saveImage(Context context, String url, Bitmap bitmap,ImageView imageView,String tag){
        //壁纸保存路径
        String imgDir = Environment.getExternalStorageDirectory().getPath() + "/PureACG";
        //图片名称处理
        ...
        try {
            ...
            if (tag.equals("save")){
                if (compress){
                    Snackbar.make(imageView,"壁纸已经躺在你的图库里啦.. ( ＞ω＜)",Snackbar.LENGTH_SHORT).show();
                }else {
                    Snackbar.make(imageView,"壁纸拒绝了你的请求.. ( ＞ω＜)",Snackbar.LENGTH_SHORT).show();
                }
            }else{
                if (!compress){
                    Snackbar.make(imageView,"壁纸拒绝了你的请求.. ( ＞ω＜)",Snackbar.LENGTH_SHORT).show();
